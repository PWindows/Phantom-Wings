...
func (h *Handler) HandleLog(e *log.Entry) error {
	...
	for _, name := range names {
		...
		if err, ok := e.Fields.Get("error").(error); ok {
			...
			// github.com/pwindows/phantom-wings/server/filesystem.(*Archive).addToArchive
			//         github.com/pwindows/phantom-wings/server/filesystem/archive.go:166
			// ... (Truncated the stack for easier reading)
			// runtime.goexit
			//         runtime/asm_amd64.s:1374
			// **NEW LINE INSERTED HERE**
			// backup: error while generating server backup
			// github.com/pwindows/phantom-wings/server.(*Server).Backup
			//         github.com/pwindows/phantom-wings/server/backup.go:84
			// ... (Truncated the stack for easier reading)
			// runtime.goexit
			//         runtime/asm_amd64.s:1374
			//
